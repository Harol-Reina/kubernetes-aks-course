...
> **De Pods Individuales a Fleets de Réplicas**: Auto-recuperación, escalado horizontal, y alta disponibilidad con ReplicaSets.
...
- **Entender ReplicaSets**: Controlador que mantiene N réplicas del mismo Pod
- **Reconciliation loop**: Cómo K8s converge estado deseado vs real
- **Selectors y labels**: Matching de Pods con ReplicaSets
- **Owner references**: Relación ReplicaSet → Pods
- **Diferencias arquitectónicas**: ReplicaSet vs ReplicationController (legacy)

### 🛠️ Objetivos Técnicos
- **Crear ReplicaSets**: Manifiestos YAML completos
- **Escalar réplicas**: kubectl scale, patch YAML
- **Label selectors**: matchLabels y matchExpressions
- **Gestionar failures**: Auto-recuperación ante crashes
- **Orphan Pods**: Eliminar sin cascada (--cascade=orphan)

### 🔍 Objetivos de Troubleshooting
- **Diagnosticar réplicas faltantes**: Por qué no se crean Pods
...
- **Alta disponibilidad**: Múltiples réplicas distribuidas
- **Self-healing**: Recuperación automática de fallos
- **Horizontal scaling**: Aumentar/reducir capacidad
- **Preparación CKA/CKAD**: ReplicaSets son base para Deployments (20% examen)
...
- ✅ **Módulos 04-05 completados**: Pods básicos y gestión avanzada
- ✅ **Labels y selectors**: Conceptos de etiquetado
- ✅ **YAML avanzado**: Templates, replicas count
- ✅ **Arquitectura K8s**: Control Plane, controllers

### Herramientas Necesarias
- 🔧 **Minikube activo**: Cluster funcionando
- 🔧 **kubectl configurado**: Con autocompletado
- 🔧 **Recursos suficientes**: Para crear 3-5 réplicas
...
# Verificar recursos (debería soportar 3-5 Pods)
kubectl describe node minikube | grep -A 5 "Allocated resources"

# Probar labels
kubectl run test --image=nginx --labels=app=test,tier=frontend
...
1. **Introducción a ReplicaSets** (20 min) - Qué son, arquitectura
2. **Reconciliation Loop** (15 min) - Cómo funciona el controller
3. **Labels y Selectors** (15 min) - Matching de Pods
4. **Diferencias con Deployments** (10 min) - Cuándo usar qué

### Contenido Práctico (120-180 minutos)
1. **Crear primer ReplicaSet** (30 min) - YAML básico
2. **Escalar réplicas** (20 min) - Manual y declarativo
3. **Auto-recuperación** (30 min) - Eliminar Pods, ver recreación
4. **Labels avanzados** (30 min) - matchExpressions
5. **Troubleshooting** (30 min) - Diagnosticar problemas
6. **Proyecto final** (30 min) - ReplicaSet production-ready

### Ejemplos Prácticos
- 📁 **01-replicaset-basico/** - Primera réplica
- 📁 **02-escalado/** - Scale up/down
- 📁 **03-labels-selectors/** - Matching avanzado
- 📁 **04-auto-recovery/** - Self-healing
- 📁 **05-orphan-pods/** - Cascading vs orphan delete
- 📁 **06-troubleshooting/** - Diagnóstico
- 📁 **07-vs-replicationcontroller/** - Comparación legacy
- 📁 **08-best-practices/** - Patrones recomendados

### Laboratorios
- 🔬 **Lab 01**: Crear ReplicaSet con 3 réplicas
- 🔬 **Lab 02**: Escalar de 3 a 5 réplicas
- 🔬 **Lab 03**: Probar auto-recuperación
- 🔬 **Lab 04**: Labels y selectors complejos
- 🔬 **Lab 05**: Troubleshooting réplicas faltantes
...
**Tiempo**: 4-5 horas
```
Día 1: Fundamentos (2.5 horas)
  ├─ Teoría: ReplicaSets (40 min)
  ├─ Lab 01: Primer ReplicaSet (40 min)
  ├─ Lab 02: Escalado manual (30 min)
  └─ Lab 03: Auto-recuperación (40 min)

Día 2: Avanzado (2 horas)
  ├─ Teoría: Labels selectors (30 min)
  ├─ Lab 04: matchExpressions (40 min)
  ├─ Lab 05: Troubleshooting (30 min)
...
Sesión única:
  ...
  ├─ Labs 01-03 (escalado + recovery) (60 min)
  ├─ Lab 04 (labels avanzados) (30 min)
...
Estrategia:
  ├─ RESUMEN-MODULO.md (20 min)
  ├─ Práctica comandos (40 min)
  │   ├─ kubectl create/scale/delete
  │   ├─ YAML template ReplicaSet
  │   └─ Labels matching
  └─ Notas: ReplicaSets SON creados por Deployments
      (en examen casi siempre usarás Deployments)
```
...
```
ejemplos/
├── 01-replicaset-basico/
│   ├── README.md
│   ├── replicaset-nginx.yaml        # 3 réplicas nginx
│   └── commands.md                  # kubectl get rs
│
├── 02-escalado/
│   ├── README.md
│   ├── scale-imperative.sh          # kubectl scale
│   └── scale-declarative.yaml       # replicas: 5
│
├── 03-labels-selectors/
│   ├── README.md
│   ├── matchLabels.yaml             # Selector simple
│   └── matchExpressions.yaml        # In, NotIn, Exists
│
├── 04-auto-recovery/
│   ├── README.md
│   ├── delete-pod-test.sh           # Eliminar Pod, ver recreación
│   └── failure-simulation.yaml      # Simular crash
│
├── 05-orphan-pods/
│   ├── README.md
│   ├── delete-cascade.sh            # Elimina RS y Pods
│   └── delete-orphan.sh             # Elimina RS, deja Pods
│
├── 06-troubleshooting/
│   ├── README.md
│   ├── replicaset-no-pods.yaml      # Selector erróneo
│   ├── diagnose-commands.md         # Troubleshooting
│   └── events-analysis.md           # Interpretar eventos
│
├── 07-vs-replicationcontroller/
│   ├── README.md
│   ├── replicaset-example.yaml      # Moderno
│   └── replicationcontroller.yaml   # Legacy (evitar)
│
└── 08-best-practices/
    ├── README.md
    ├── good-replicaset.yaml         # Patrón correcto
    └── use-deployment-instead.md    # Por qué Deployments > RS
```
...
```
� Práctica con réplicas    50%  ██████████▓░░░░░░░░░
📖 Teoría controllers       30%  ██████▓░░░░░░░░░░░░░
🔍 Troubleshooting          15%  ███▓░░░░░░░░░░░░░░░░
🎯 Patterns avanzados       5%   █▓░░░░░░░░░░░░░░░░░░
```

### Enfoque Pedagógico
1. **Experimentación**: Crear, romper, reparar
2. **Observación**: Ver reconciliation loop en acción
3. **Comparación**: ReplicaSet vs Pod individual
4. **Context setting**: Por qué Deployments > ReplicaSets

### Flujo de Trabajo
```
1. Crear RS → 2. Ver Pods crearse → 3. Eliminar 1 Pod → 4. Ver recreación
                ↓
5. Escalar a 5 → 6. Ver nuevos Pods → 7. Escalar a 1 → 8. Ver eliminación
```
...
- ➡️ **Módulo 07**: Deployments (gestionan ReplicaSets)
- ➡️ **Módulo 08**: Services (balanceo entre réplicas)
- ➡️ **Módulo 10**: Namespaces (ReplicaSets por namespace)
- ➡️ **Módulo 11**: Resource Limits (cuotas de réplicas)

### Relación con Módulos Anteriores
```
Módulo 04-05: 1 Pod individual
    ↓
Módulo 06: N réplicas del mismo Pod ← ESTÁS AQUÍ
    ↓
Módulo 07: Deployments (gestión de ReplicaSets)
```

---

## 💡 Concepto Clave: Reconciliation Loop

**El corazón de K8s es converger estado deseado → estado real**

```
┌──────────────────────────────────────┐
│   Reconciliation Loop (cada 5-10s)   │
├──────────────────────────────────────┤
│  1. Lee YAML: "spec.replicas: 3"     │
│  2. Cuenta Pods reales: 2 running    │
│  3. Detecta diferencia: 3 - 2 = 1    │
│  4. ACTÚA: Crea 1 Pod adicional      │
│  5. Espera 5-10 segundos             │
│  6. Vuelve al paso 1 (infinito)      │
└──────────────────────────────────────┘
```

**Esto significa**:
- ✅ Auto-recuperación: Si Pod muere → se recrea
- ✅ Auto-escalado: Cambias replicas → se ajusta
- ✅ Auto-corrección: Editas labels → se reasigna
...
- ✅ **Comprender ReplicaSets** y su arquitectura interna
- ✅ **Crear ReplicaSets** con manifiestos YAML
- ✅ **Escalar réplicas** manual y declarativamente
- ✅ **Usar label selectors** (matchLabels, matchExpressions)
- ✅ **Entender reconciliation loop** del controller
- ✅ **Diagnosticar réplicas faltantes** y problemas de scheduling
- ✅ **Aplicar auto-recuperación** ante fallos de Pods
- ✅ **Diferenciar ReplicaSet vs Deployment** (cuándo usar qué)
